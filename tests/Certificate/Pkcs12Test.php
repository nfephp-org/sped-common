...
';
    ...
    public $pathCerts = '../fixtures/certs/';
    public $pathXml = '../fixtures/xml/';
    
    public function __construct()
    {
        $this->pathCerts = dirname(dirname(__FILE__)) . '/fixtures/certs/';
        $this->pathXml = dirname(dirname(__FILE__)) . '/fixtures/xml/';
    }
    
    public function testCarregarPfx()
    {
        $pathCerts = $this->pathCerts;
        $cnpj = '99999090910270';
        $pubKey = '';
        $priKey = '';
        $certKey = '';
        $pkcs = new Pkcs12($pathCerts, $cnpj, $pubKey, $priKey, $certKey);
        $arquivopfx = dirname(dirname(__FILE__)) . '/fixtures/certs/certificado_teste.pfx';
    ...
    public function testCarregarPfxFile()
    {
        ...
        $certKey = '';
        $pkcs = new Pkcs12($pathCerts, $cnpj, $pubKey, $priKey, $certKey);
        $arquivopfx = $pathCerts.'certificado_teste.pfx';
        $keyPass = 'associacao';
        $createFiles = false;
        $ignoreValidity = true;
        $ignoreOwner = true;
    ...
    public function testAssinarXml()
    {
        ...
        $xmlpath = $this->pathXml.'NFe/35101158716523000119550010000000011003000000-nfe.xml';
        $docxml = file_get_contents($xmlpath);
        $signedxmlpath = $this->pathXml.'NFe/signed-nfe.xml';
    ...
    public function testVerificarAssinatura()
    {
        $cnpj = '99999090910270';
        $pathCerts = $this->pathCerts;
        $pkcs = new Pkcs12($pathCerts, $cnpj, '', '', '', true);
        $signedxmlpath = $this->pathXml.'NFe/signed-nfe.xml';
    ...
    {
        $aCerts[] = $this->pathCerts.'ACCertisignG6_v2.cer';
        $aCerts[] = $this->pathCerts.'ACCertisignMultiplaG5.cer';
        $aCerts[] = $this->pathCerts.'ACRaizBrasileira_v2.cer';
        $cert = $this->priPem . "\r\n" .
            $this->pubPem . "\r\n" .
            $this->accg6 . "\r\n" .
            $this->accmg5 . "\r\n" .
